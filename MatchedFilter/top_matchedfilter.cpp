...
#pragma HLS ARRAY_PARTITION variable=rxmat block factor=512 dim=2
	cdt static rxmat_delay[dim_r][dim_c];

	cdt static xmat[dim_c][dim_r];
#pragma HLS ARRAY_PARTITION variable=xmat cyclic factor=512 dim=1

	cdt mulOut[dim_r]; // Calculating output for Complex Data type
#pragma HLS ARRAY_PARTITION variable=mulOut block factor=128 dim=1
