...
typedef struct {
    ...
    XMatmult_accel_uio_map maps[ MAX_UIO_MAPS ];
} XMatmult_accel_uio_info;

/***************** Variable Definitions **************************************/
static XMatmult_accel_uio_info uio_info;
...
static int uio_info_read_name(XMatmult_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMatmult_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMatmult_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMatmult_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMatmult_accel_Initialize(XMatmult_accel *InstancePtr, const char* InstanceName) {
	XMatmult_accel_uio_info *InfoPtr = &uio_info;
